...
export const SEASON_TRADITIONAL_COLUMNS: SeasonBoxscoreColumn[] = [
  ...
  { key: "pts", label: "PTS", format: (c, mode) => formatDecimal(c.scaled.pts, countDigits(mode)), value: (c) => c.scaled.pts },
  {
    key: "fgm",
    label: "FGM",
...
export const SEASON_SCORING_COLUMNS: SeasonBoxscoreColumn[] = [
  ...
  },
  {
    key: "pct2pm",
    label: "%2PM",
    format: (c) => formatPct100(sharePct(c.raw.fgm - c.raw.tpm, c.team.fgm - c.team.tpm)),
    value: (c) => sharePct(c.raw.fgm - c.raw.tpm, c.team.fgm - c.team.tpm),
  },
  {
    key: "pct2pa",
    label: "%2PA",
    format: (c) => formatPct100(sharePct(c.raw.fga - c.raw.tpa, c.team.fga - c.team.tpa)),
    value: (c) => sharePct(c.raw.fga - c.raw.tpa, c.team.fga - c.team.tpa),
  },
  {
    key: "pctftm",
    label: "%FTM",
 ...
 * computeTeamRatingsを呼ぶだけ）。computeGamePeriodTotals（選手個人版）用の共通部分を
 * ここに切り出した
 */
...
/**
 * チーム詳細ページ「比較」タブ・「チームスタッツ」タブ（Phase H4）用。buildTeamGameBoxTotals
 * （1試合分）を複数試合分呼び、own/opp双方のBoxscoreCounts・プレータイプ内訳を合算した上で
 * 「1試合あたり平均」に変換してCOLUMNS_BY_TABにそのまま渡せるColumnCtxを組み立てる。合計値の
 * まま比較するとスロットごとの試合数の違いで比較しづらくなるため、個人詳細ページの比較タブ
 * （seasonBoxCompareDefs、常に「平均」固定）と同じ方針にした。POSSは6章・11章・53章と同じ
 * 「試合単位で確定させてから合算する」方針（非線形性回避）を踏襲し、
 * PACE/ORtg/DRtg/NetRtgは合算後のPOSS・PTSから再計算する（比率のため合算値・平均値のどちらから
...
export interface TeamPointsBreakdown {
  bench: number;
  starter: number;
  /** 日本人選手の得点。internationalと合わせて登録区分の2分割になる（src/lib/classificationFilter.ts参照） */
  japanese: number;
  /** 外国籍・帰化・アジア特別枠選手の得点 */
...
function rawTeamPointsBreakdown(rows: BoxscoreRow[], masterById: Map<string, PlayerMasterEntry>): TeamPointsBreakdown {
  ...
  for (const r of rows) {
    ...
    else if (classification !== undefined) international += r.Point;
  }
