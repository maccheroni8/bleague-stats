 ...
 */
export function CompositionPieChart({ title, segments, size = 220, valueDigits = 1 }: CompositionPieChartProps) {
  ...
  const renderLabel = (props: unknown) => {
    ...
    if (!seg) return null;
    const bend = polarToCartesian(p.cx, p.cy, outerRadius + 14, p.midAngle);
    const end = polarToCartesian(p.cx, p.cy, outerRadius + 30, p.midAngle);
